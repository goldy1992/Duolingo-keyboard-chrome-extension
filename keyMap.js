function transformCodeTypedCharacterUKR(typedChar) 
{
	switch(typedChar)
	{
...
function transformCodeTypedCharacterRUS(typedChar) 
{
	console.log("called Russian");
	switch(typedChar)
	{
		case 81: return String.fromCharCode(1049); // Q -> Й
		case 113: return String.fromCharCode(1081);	// q -> й
		...
		case 125: return String.fromCharCode(0x42A); // ъ
		case 93: return String.fromCharCode(0x44A);	// Ъ		
		
		case 65: return String.fromCharCode(1060); // Ф
		case 97: return String.fromCharCode(1092);	// ф	
		
		case 83: return String.fromCharCode(0x42B); // ы
		case 115: return String.fromCharCode(0x44B);	//Ы	
		...
		// english
		case 64: return String.fromCharCode(0x42D); // Ээ
		// american
		case 34: return String.fromCharCode(0x42D); // Є
		case 39: return String.fromCharCode(0x44D);	// є
		...
		// english
		case 0xAC: return String.fromCharCode(0x401);

		// anmrican
		case 0x7E: return String.fromCharCode(0x401);		
		case 96: console.log("return `"); return String.fromCharCode(0x451); // ,
		case 47: console.log("return ."); return "."; // .		
		case 38: return "?"; // .			
...
function editText(id, newChar)
{
	...
	var element = $("#" + id + "");
	console.log("start: " + element.prop("selectionStart"));
	console.log("end: " + element.prop("selectionEnd"));

	var cursorPosition = element.prop("selectionStart");
		var cursorEndPosition = element.prop("selectionEnd");
  var tx = element.val();
	//console.log("tx: " + tx);  
	var beforeChar = tx.substring(0, cursorPosition);		
	var afterChar = tx.substring(cursorEndPosition, tx.length);	
...
function keyListener(evt)
{
		...
		(focused.is("input") && focused.attr("placeholder").includes("Ukrainian")))
		{
			//console.log("focused: " + focused.get(0).tagName);
			var x = transformCodeTypedCharacterUKR(evt.which);
			if (x !== null)
			{
				evt.preventDefault();
		...
		else if ((focused.is("textarea") && focused.attr("lang") === "ru") ||
		(focused.is("input") && focused.attr("placeholder").includes("Russian")))
		{
			//console.log("focused: " + focused.get(0).tagName);
			var x = transformCodeTypedCharacterRUS(evt.which);
			if (x !== null)
			{
				evt.preventDefault();
				//console.log("focused: " + focused.attr("id"));				
